...
PREF=test
DOCUT=1
CUTOFF=700  #should be a parameter, later...
PREDFILE=""
PREDMAP=""
PREDLAT=""
GOLDFILE=""
...
echo "###########################"
echo "\n"
TEMP=`getopt -o DnlupcAHy:tdks:m:L:g:b:e:P:G: --long help,debug,new,labeled,unlabeled,ptb,conll,Arabic,Hebrew,any:,test,dev,cut,predfile:,predmap:,predlat:,gold:,begin:,end:,spmrldata_rootdir,fivek,P:,G: -- "$@"`
...
while true; do
  case "$1" in
    ...
    -n | --new )   TEDEVALJAR=$PROGDIR/tedeval-2.2.jar; shift ;;
        -P | --P ) PREDSEG="$2" ; shift 2;;
        -G | --G ) GOLDSEG="$2" ; shift 2;;
...
java  -Xmx768m -jar $TEDEVALJAR $LABEL -g  $GOLDFILE.4tedeval.$$ -sg $GOLDSEG.4tedeval.$$ -p $PREDFILE.4tedeval.$$ -sp $PREDSEG.4tedeval.$$ -format $TYPENAME -o $PREDFILE.4tedeval.simple_tedeval.res$SUF
...
echo "java  -Xmx768m -jar $TEDEVALJAR $LABEL -g  $GOLDFILE.4tedeval.$$ -sg $GOLDSEG.4tedeval.$$ -p $PREDFILE.4tedeval.$$ -sp $PREDSEG.4tedeval.$$ -format $TYPENAME -o $PREDFILE.4tedeval.simple_tedeval.res$SUF" > /dev/stderr
echo " "
...
rm -f $GOLDFILE.4tedeval.$$ $PREDFILE.4tedeval.$$ $GOLDSEG.4tedeval.$$ $PREDSEG.4tedeval.$$
