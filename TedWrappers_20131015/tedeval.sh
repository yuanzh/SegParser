...
echo "###########################"
echo "\n"
TEMP=`getopt -o DnlupcAHy:tdks:m:L:g:b:e: --long help,debug,new,labeled,unlabeled,ptb,conll,Arabic,Hebrew,any:,test,dev,cut,predfile:,predmap:,predlat:,gold:,begin:,end:,spmrldata_rootdir -- "$@"`
...
while true; do
  case "$1" in
	...
	-A | --arabic ) LANGUAGE=ARABIC ;SUF=""; SKIPFILE=""; echo "ici arabic" ; exit ; shift ;; 
...
if [ -z "$PREDLAT" ] ; then
	true
	#PREDMAP=""   # PREDMAP should be empty
	if [ -z "$PREDMAP" ] ; then
		PREDLAT=${SPMRLDATA_ROOTDIR}/$LDIR/pred/$TYPE/${PREF}/${PREF}.${LANGUAGE}.pred.$TYPE.tobeparsed.pred_tagged+pred_token.disamb.lattices
	else
		PREDLAT=$PREDFILE
	fi
fi
...
if [ -z "$GOLDFILE" ] ; then
	GOLDFILE=${PREF}.${LANGUAGE}.gold.$TYPE
	GOLDLAT=${PREF}.${LANGUAGE}.gold.$TYPE.tobeparsed.gold_tagged+gold_token.lattices
else
	GOLDLAT=$GOLDFILE.tobeparsed.gold_tagged+gold_token.lattices
fi
...
if [ -f "$GOLDLAT" ] ; then
	echo "gold mapping: $GOLDLAT found"
else
	echo "gold mapping: $GOLDLAT not found"
	exit
fi
...
if [ -f "$PREDLAT" ] ; then
	echo "pred lat file: $PREDLAT found"
else
	echo "pred lat file: $PREDLAT not found"
	exit
fi

if [ -f "$PREDMAP" ] ; then
	echo "predmap: $PREDMAP found"
else
	echo "predmap: $PREDMAP not found, will be generated from $PREDLAT"
fi
...
if test $DOCUT = 1 ; then
	...
	cat $GOLDLAT | perl -pe 's/^\s*$/\n/' |./get_cutoffed_sent.pl -c -K 70 /dev/null > $GOLDLAT.tobeskipped
	if test "$PREF.$LANGUAGE" = "test.Arabic" ; then
		#echo -e "\n799" >> $GOLDLAT.tobeskipped # buggy sentence for unlabeld evaluation on arabic test
		echo "\n" >>  $GOLDLAT.tobeskipped
	elif test "$PREF.$LANGUAGE" = "dev.Arabic" ; then
		#echo -e "\n904\n1889" >>  $GOLDLAT.tobeskipped #buggy sentence for unlabeld evaluation on arabic dev
		echo "\n" >>  $GOLDLAT.tobeskipped

	fi
	SKIPFILE=$GOLDLAT.tobeskipped
else
	SKIPFILE=/dev/null
fi
...
echo "generating mapping files "
echo -e "\t==> gold"

wc -L $GOLDLAT
cat $GOLDLAT|  perl -pe 's/^\s*$/\n/' |./skip_lines.pl -c $SKIPFILE  | perl -pe 's/\:/<column>/g; s/-(..)B-/$1B/g' | ./lines -c $START $END |tee $GOLDLAT.debug |./lattice_to_segmentation.py -$TYPE|perl -pe 's/^\s*$/\n/' |uniq > $GOLDLAT.4tedeval.mapping.$$
echo "cat $GOLDLAT |./skip_lines.pl $ARG $SKIPFILE  | perl -pe 's/\:/<column>/g; s/-(..)B-/$1B/g' | ./lines -c $START $END |tee $GOLDLAT.debug | perl -pe 's/^\s*$/\n/' |./lattice_to_segmentation.py -$TYPE|perl -pe 's/^\s*$/\n/' |uniq > $GOLDLAT.4tedeval.mapping.$$"
wc -L $GOLDLAT.4tedeval.mapping.$$
...
if [ -z $PREDMAP ]; then 
	echo "DANS -z predmap"
	cat $PREDLAT |perl -pe 's/^\s*$/\n/' |./skip_lines.pl -c $SKIPFILE | perl -pe 's/\:/<column>/g; s/-(..)B-/$1B/g' | ./lines -c $START $END |tee $PREDLAT.debug | perl -pe 's/^\s*$/\n/' |./lattice_to_segmentation.py -$TYPE |perl -pe 's/^\s*$/\n/' |uniq > $PREDLAT.4tedeval.mapping.$$
	PREDMAP=$PREDLAT.4tedeval.mapping.$$
	echo "cat $PREDLAT |perl -pe 's/^\s*$/\n/' |./skip_lines.pl $ARG $SKIPFILE | perl -pe 's/\:/<column>/g; s/-(..)B-/$1B/g' | ./lines -c $START $END |tee $PREDLAT.debug | perl -pe 's/^\s*$/\n/' |./lattice_to_segmentation.py -$TYPE |perl -pe 's/^\s*$/\n/' |uniq > $PREDLAT.4tedeval.mapping.$$" 
	wc -L  $PREDLAT.4tedeval.mapping.$$
else
	echo "SINON -z predmap"
	#cat $PREDMAP ||perl -pe 's/^\s*$/\n/' | skip_lines.pl -c $SKIPFILE |lines -c $START $END| perl -pe 's/^\s*$/\n/'|uniq > $PREDLAT.4tedeval.mapping.$$
	echo ""
	cat $PREDMAP |perl -pe 's/^\s*$/\n/' | ./skip_lines.pl -c $SKIPFILE |./lines -c $START $END > $PREDMAP.4tedeval.mapping.$$
	PREDMAP=$PREDMAP.4tedeval.mapping.$$
	if [ ! -f $PREDMAP ] ; then 
		echo "MERDE" 
		exit
	fi
	echo "PREDMAP = $PREDMAP"
	#exit
...
java  -Xmx2g -jar $TEDEVALJAR $LABEL -g  $GOLDFILE.4tedeval.$$ -sg $GOLDLAT.4tedeval.mapping.$$ -p $PREDFILE.4tedeval -sp $PREDMAP -format $TYPENAME -o $PREDFILE.4tedeval.evalted.res$LABEL
...
echo "java -server -Xmx768m -jar $TEDEVALJAR $LABEL -g  $GOLDFILE.4tedeval.$$ -sg $GOLDLAT.4tedeval.mapping.$$ -p $PREDFILE.4tedeval -sp $PREDMAP -format $TYPENAME -o $PREDFILE.4tedeval.tedeval.res$SUF"
echo "\n\n"
#rm -f $GOLDFILE.4tedeval.$$ $GOLDLAT.4tedeval.mapping.$$ $PREDMAP
